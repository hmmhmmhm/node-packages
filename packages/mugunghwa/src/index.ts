...
let nonFilteredCharMap: string[] = [];

// 인코딩 테이블이 여기에 담깁니다.
let encodeTable: string[] = [];

// 오타 정정을 위한 문자보수 테이블이 여기에 담깁니다.
let charFixMap: Record<string, string[]> = {};

// 단어로 구성된 중간자가 여기에 담깁니다.
let seperatorWord: string[] = [];

// 압축된 글자목록입니다.
const compressedCharMap = 'hkNhyGwtDYJ02BdNgvTYI82B92wGe2A1OwNZ2AquwLN2A-';

const parsedCharMap = LZString.decompressFromEncodedURIComponent(compressedCharMap);
if (parsedCharMap) {
  nonFilteredCharMap = parsedCharMap.split('');
}

// 압축된 중간자목록입니다.
const compressedSeperatorWord = 'idNh2hwAzeg2Q2gDQ0A';

// 압축된 중간자목록을 불러옵니다.
const parsedSeperatorWord = LZString.decompressFromEncodedURIComponent(compressedSeperatorWord);
if (parsedSeperatorWord) {
...
export function encode(index: number, separator: string = "-"): string | null {
  ...
  // 인코딩 되는 글자길이 확인
  let maxLength = expectLength(index);
  ...
  // 10진수를 다진수 배열로 변환
  let matrix = multipleBase(max, index);
  ...
  if (max.length == 1) {
    ...
    for (let i = 0; i < pairGroupLength; i++) {
      ...
      if (pairGroup.length == 2) {
        let pairIndex = decimalBase([72, 72], pairGroup);
...
export function decode(code: string, separator: string = "-"): number | null {
    ...
    for (let i = 0; i <= (pairLength - 1); i++) {
      ...
      if (pairGroup.length == 2) {
        ...
        let pairMatrix = multipleBase([72, 72], pairIndex);
    ...
    // 10진수 색인번호로 변환합니다.
    let decodedIndex = decimalBase(max, decodedMatrix);
